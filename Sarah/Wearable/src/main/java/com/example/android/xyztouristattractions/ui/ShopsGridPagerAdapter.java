 ...
 * {@link ShopsActivity}.
 */
public class ShopsGridPagerAdapter extends GridPagerAdapter
    ...
    public ShopsGridPagerAdapter(
            Context context, ArrayList<Shop> shops) {
        super();
        mContext = context;
    ...
    public int getRowCount() {
        return (mShops != null && mShops.size() > 0) ? mShops.size() : 1;
    }
    ...
    public Object instantiateItem(ViewGroup container, int row, final int column) {
        if (mShops != null && mShops.size() > 0) {
            final Shop shop = mShops.get(row);
            switch (column) {
                case PAGER_PRIMARY_IMAGE_COLUMN:
                case PAGER_SECONDARY_IMAGE_COLUMN:
                    // Two pages of full screen images, one with the shop name
                    // and one with the distance to the shop
                    ...
                    if (column == PAGER_PRIMARY_IMAGE_COLUMN) {
                        imageView.setImageResource(shop.imageUrl);
                        textView.setText(shop.name);
                    } else {
                        imageView.setImageResource(shop.secondaryImageUrl);
                        if (TextUtils.isEmpty(shop.distance)) {
                            overlayTextLayout.setVisibility(View.GONE);
                        } else {
                            textView.setText(mContext.getString(
                                    R.string.map_caption, shop.distance));
                        }
                    }
                ...
                case PAGER_DESCRIPTION_COLUMN:
                    ...
                    descTextView.setText(shop.description);
                ...
                case PAGER_NAVIGATE_ACTION_COLUMN:
                    ...
                    navActionPage.setOnClickListener(getStartActionClickListener(
                            shop, Constants.START_NAVIGATION_PATH,
                ...
                case PAGER_OPEN_ACTION_COLUMN:
                    ...
                    openActionPage.setOnClickListener(getStartActionClickListener(
                            shop, Constants.START_SHOP_PATH,
     ...
     *
     * @param shop The shop to start the action on
     ...
     */
    private void startAction(Shop shop, String pathName, int confirmAnimationType) {
        ...
        UtilityService.startDeviceActivity(mContext, pathName, shop.name, shop.city);

        ((Activity)mContext).finish();
    }

    /**
     * Helper method to generate the OnClickListener for the shop actions.
     */
    private View.OnClickListener getStartActionClickListener(final Shop shop,
            final String pathName, final int confirmAnimationType) {
        View.OnClickListener clickListener = new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startAction(shop, pathName, confirmAnimationType);
            }
        };

...
public class UtilityService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (ACTION_ADD_GEOFENCES.equals(action)) {
            ...
            String city = currentLocation == null ? Shops.TEST_CITY :
                    Shops.getClosestCity(currentLocation);
    ...
    private void addGeofencesInternal() {
        ...
        if (connectionResult.isSuccess() && googleApiClient.isConnected()) {
            ...
            GeofencingApi.addGeofences(googleApiClient,
                    Shops.getGeofenceList(), pendingIntent);
    ...
    private void showNotification(String cityId, boolean microApp) {
        ...
        List<Shop> shops = SHOPS.get(cityId);

        if (microApp) {
            // If micro app we first need to transfer some data over
            sendDataToWearable(shops);
        }

        // The first (closest) tourist shop
        Shop shop = shops.get(0);

        // Limit shop to send
        int count = shops.size() > Constants.MAX_SHOPS ?
                Constants.MAX_SHOPS : shops.size();

        // Pull down the tourist shop images from the network and store
        HashMap<String, Bitmap> bitmaps = new HashMap<>();
        try {
            for (int i = 0; i < count; i++) {
                bitmaps.put(shops.get(i).name,
                        Glide.with(this)
                                .load(shops.get(i).imageUrl)
                                .asBitmap()
        ...
        PendingIntent pendingIntent = PendingIntent.getActivity(this, 0,
                DetailActivity.getLaunchIntent(this, shop.name),
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
                .setStyle(new NotificationCompat.BigPictureStyle()
                                .bigPicture(bitmaps.get(shop.name))
                                .setBigContentTitle(shop.name)
     ...
     * @param shops list of shop data to transfer over
     */
    private void sendDataToWearable(List<Shop> shops) {
        GoogleApiClient googleApiClient = new GoogleApiClient.Builder(this)
                .addApi(Wearable.API)
                .build();
        ...
        ArrayList<DataMap> shopsData = new ArrayList<>(count);
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                // Fetch and resize shop image bitmap
            ...
            if (image != null && secondaryImage != null) {
                ...
                DataMap shopData = new DataMap();

                String distance = Utils.formatDistanceBetween(
                        Utils.getLocation(this), shop.location);

                shopData.putString(Constants.EXTRA_TITLE, shop.name);
                shopData.putString(Constants.EXTRA_DESCRIPTION, shop.description);
                shopData.putDouble(
                        Constants.EXTRA_LOCATION_LAT, shop.location.latitude);
                shopData.putDouble(
                        Constants.EXTRA_LOCATION_LNG, shop.location.longitude);
                shopData.putString(Constants.EXTRA_DISTANCE, distance);
                shopData.putString(Constants.EXTRA_CITY, shop.city);
                shopData.putAsset(Constants.EXTRA_IMAGE,
                        Utils.createAssetFromBitmap(image));
                shopData.putAsset(Constants.EXTRA_IMAGE_SECONDARY,
        ...
        if (connectionResult.isSuccess() && googleApiClient.isConnected()
            ...
            PutDataMapRequest dataMap = PutDataMapRequest.create(Constants.SHOP_PATH);
            dataMap.getDataMap().putDataMapArrayList(Constants.EXTRA_SHOPS, shopsData);
